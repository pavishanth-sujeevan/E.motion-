"""
Train Tamil classifier with augmented data
"""
...
def train_model(train_loader, val_loader, model, device, num_epochs=100, patience=10):
...
def evaluate_model(model, test_loader, device):
    ...
    # Per-class accuracy
    emotion_map = {0: 'angry', 1: 'fear', 2: 'happy', 3: 'neutral', 4: 'sad'}
    print("\nPer-class accuracy:")
    for i in range(5):
        mask = all_labels == i
        if mask.sum() > 0:
            class_acc = 100. * (all_preds[mask] == all_labels[mask]).sum() / mask.sum()
            print(f"  {emotion_map[i]:8s}: {class_acc:6.2f}% ({mask.sum()} samples)")
    
    return accuracy, all_preds, all_labels

def main():
    print("=" * 70)
    print("TRAINING TAMIL CLASSIFIER WITH AUGMENTED DATA")
    print("=" * 70)
    print()
    ...
    feature_dir = '../features/tamil_augmented'
    ...
    # Create model
    model = EmotionClassifier(input_dim=100, num_classes=5).to(device)
    ...
    print("-" * 70)
    model = train_model(train_loader, val_loader, model, device, num_epochs=100, patience=10)
    ...
    model_path = os.path.join(model_dir, 'tamil_augmented_classifier.pt')
    ...
    print(f"Augmented + Features:   {test_acc:.2f}%")
    improvement = test_acc - 34.04
    print(f"Improvement:            {improvement:+.2f}%")
    
    if test_acc > 34.04:
        print("\n✓ Model improved over baseline!")
    else:
        print("\n⚠ Model did not improve over baseline")
        print("  This suggests the feature extraction method may not be optimal")
        print("  Consider using emotion2vec's actual encoder for better results")
